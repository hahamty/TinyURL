...
public class MainVerticle extends AbstractVerticle {
    ...
    public void start(Future<Void> startFuture) throws Exception {
        ...
        eventBus.<JsonObject>rxSend("get-machine-id", new JsonObject().put("mac-address", macAddress))
                .map(Message::body)
                .subscribe(
                        messageBody -> {
                            ...
                            // init mySQLClient
                            mySQLClient = MySQLClient.createShared(vertx, new JsonObject().put("username", "root").put("password", "123").put("database", "tinyurl"));
                ...
                array -> {
                    ...
                    if (array.size() == 0 || array.getString(0) == null) {
                        mySQLClient.rxGetConnection().subscribe(
                                connection -> {
                                    connection.rxQueryWithParams("SELECT longUrl FROM Url WHERE shortUrl=?", new JsonArray().add(shortUrl)).subscribe(
                                            resultSet -> {
                                                ...
                                                response.putHeader(HttpHeaders.CONTENT_TYPE.toString(), "application/json").end(new JsonObject().put("result", longUrl).toString());
                    ...
                    response.putHeader(HttpHeaders.CONTENT_TYPE.toString(), "application/json").end(new JsonObject().put("result", json.getString("longUrl")).toString());
                },
                e -> {
                    // TODO
                }
    ...
    private void longUrlToShortUrl(RoutingContext routingContext) {
        ...
        mySQLClient.rxGetConnection().subscribe(
                connection -> {
                    connection.rxUpdateWithParams("INSERT INTO Url(shortUrl, longUrl) VALUE (?, ?)", new JsonArray().add(shortUrl).add(longUrl)).subscribe(
                            updateResult -> {
                                if (updateResult.getKeys().size() != 1) {
                                    // TODO
                                }
                                response.putHeader(HttpHeaders.CONTENT_TYPE.toString(), "application/json").end(new JsonObject().put("result", shortUrl).toString());
                                JsonObject json = new JsonObject().put("shortUrl", shortUrl).put("longUrl", longUrl);
                                redisClient.rxHmset("url", new JsonObject().put(shortUrl, json.toString())).subscribe();
                            },
                            e -> {
